(function (namespace, $) {
	...
	p._changeValues = function(data){
		...
		if (data.finalSales == '' || data.finalSales == 'undefined') {
			data.finalSales == 0.00;
		}
		...
		if (data.grossSales > 0) {
			$('#grossSaleToday').text(currCode + " " + currencyFormat(data.grossSales,currencyCode != 'INR'?'USD':'INR'));
		}else{
			$('#grossSaleToday').text(currCode + " " + data.grossSales);
		}

		if (data.netSales > 0) {
			$('#netSaleToday').text(currCode + " " + currencyFormat(data.netSales,currencyCode != 'INR'?'USD':'INR'));
		}else{
			$('#netSaleToday').text(currCode + " " + data.netSales);
		}

		if (data.finalSales > 0) {
			$('#finalSaleToday').text(currCode + " " + currencyFormat(data.finalSales,currencyCode != 'INR'?'USD':'INR'));
		}else{
			$('#finalSaleToday').text(currCode + " " + data.finalSales);
		}
		...
		// $('#orderPerformance').text(orderChange + '%');
	 	...
	 	if (data.payment.length > 0) {
		 	for (var i = 0; i < data.payment.length; i++) {
		 		...
		 		if (paymentSales > 0) {
		 			paymentSales = currencyFormat(paymentSales);
		 		}
			...
			for (var i = 0; i < data.topItemSales.length; i++) {
				...
				$('#prodQuant' + j).text(Math.round(varCount));
			...
			for (var i = 0; i < data.topCatSales.length; i++) {
				...
				$('#catQuant' + k).text(Math.round(catCount));
			...
			for (var g = k; g < 6; g++) {
	 		...
	 		var k = 1;
			for (var i = 0; i < data.topStoreSales.length; i++) {
				var storeName = data.topStoreSales[i].storeName;
				
				var storeGross = data.topStoreSales[i].grossSales;
				var storeNet = data.topStoreSales[i].netSales;
				var storeSales = data.topStoreSales[i].finalSales;
				if (storeGross > 0) {
					storeGross = currencyFormat(storeGross);
				}

				if (storeNet > 0) {
					storeNet = currencyFormat(storeNet);
				}
				var getDiv = '';
				// if (i == 0) {
				...
				$('#storeName' + k).text(storeName);
				// $('#storeGross' + k).text(currCode + " " +storeGross);
				// $('#storeNet' + k).text(currCode + " " +storeNet);
				$('#storeSales' + k).text(currCode + " " +storeSales);
